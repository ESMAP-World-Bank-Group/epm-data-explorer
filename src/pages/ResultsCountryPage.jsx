...
export default function ResultsCountryPage() {
  ...
  const buildEvolution=()=>{
    ...
    if(ind.source==='trade'){const multi=activeSc.length>1;const datasets=[];for(let i=0;i<activeSc.length;i++){const scen=activeSc[i];const col=SCEN_COLORS[i%SCEN_COLORS.length];datasets.push({label:multi?`${scen} — Imp.`:'Imp.',type:'bar',data:allYears.map(y=>+(Object.values(getTradeValsC(resultsData,scen,visZones,y)).reduce((s,v)=>s+v.imp,0)).toFixed(0)),backgroundColor:hexA(col,0.75),borderWidth:0,stack:scen});datasets.push({label:multi?`${scen} — Exp.`:'Exp.',type:'bar',data:allYears.map(y=>+(-(Object.values(getTradeValsC(resultsData,scen,visZones,y)).reduce((s,v)=>s+v.exp,0))).toFixed(0)),backgroundColor:hexA(col,0.40),borderWidth:0,stack:scen});datasets.push({label:multi?`${scen} — Net`:'Net',type:'line',data:allYears.map(y=>+(Object.values(getTradeValsC(resultsData,scen,visZones,y)).reduce((s,v)=>s+v.net,0)).toFixed(0)),borderColor:col,borderWidth:1.5,pointRadius:0,tension:0.3,fill:false,stack:`__net_${i}__`});}return{labels:allYears,datasets};}
