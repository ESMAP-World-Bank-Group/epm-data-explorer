...
import {
  ...
  processTransmissionResults, processPlants,
  computeCentroid, normalizeFuel, EPM_FUEL_COLORS, resultYears,
} from '../utils/epmFetch';

function fmtBig(n){if(!n)return'—';if(Math.abs(n)>=1e6)return`${(n/1e6).toFixed(1)}M`;if(Math.abs(n)>=1e3)return`${(n/1e3).toFixed(1)}k`;return n.toFixed(0);}
...
const INDICATORS_C = [
  ...
  { key:'Costs', label:'Costs (m USD)', source:'yearlyZone', unit:'m USD' },
...
export default function ResultsCountryPage() {
  ...
  useEffect(()=>{
    ...
    Promise.all([fetchEpmCSV(branch,dataFolder,'zcmap.csv'),fetchZonesGeoJSON(branch,dataFolder),fetchEpmCSV(branch,dataFolder,'pHours.csv')]).then(([zc,zGJ,hr])=>{
      setZcmapRows(zc||[]);setZonesGJ(zGJ);if(hr)setHoursData(processHours(hr));
    });
  },[region]);
  ...
  useEffect(()=>{
    ...
    fetchGitHubDir(region.epm.branch,`epm/output/${simRun}`).then(items=>{
      ...
      setScenarioList(s);if(s.length)setScenario(s[0]);
    });
  },[region,simRun]);
  ...
  useEffect(()=>{
    ...
    Promise.all(scenarioList.map(async scen=>{
      const[tf,yz,dp,pr,tx,pl]=await Promise.all([fetchResultCSV(branch,simRun,scen,'pTechFuelMerged.csv'),fetchResultCSV(branch,simRun,scen,'pYearlyZoneMerged.csv'),fetchResultCSV(branch,simRun,scen,'pDispatchComplete.csv'),fetchResultCSV(branch,simRun,scen,'pHourlyPrice.csv'),fetchResultCSV(branch,simRun,scen,'pTransmissionMerged.csv'),fetchResultCSV(branch,simRun,scen,'pPlantMerged.csv')]);
      return{scen,techFuel:tf?processTechFuel(tf):{},yearlyZone:yz?processYearlyZone(yz):{},dispatch:dp?processDispatchResults(dp):{},price:pr?processHourlyPrice(pr):{},transmission:tx?processTransmissionResults(tx):{},plants:pl?processPlants(pl):[]};
    })).then(res=>{
      const rd=Object.fromEntries(res.map(r=>[r.scen,r]));setResultsData(rd);
      const yrs=resultYears(res[0]?.techFuel||{});if(yrs.length)setRefYear(yrs[0]);
  ...
  const activeDispZone = dispZone||countryZoneIds[0]||null;
  ...
  const dispAvailSeasons=useMemo(()=>{const qs=new Set();for(const z of Object.values(firstDispatch))for(const q of Object.keys(z))qs.add(q);return[...qs].sort();},[firstDispatch]);
  const dispAvailDays   =useMemo(()=>{const ds=new Set();for(const z of Object.values(firstDispatch))for(const q of Object.values(z))for(const d of Object.keys(q))ds.add(d);return[...ds].sort();},[firstDispatch]);
  ...
  useEffect(()=>{
    ...
    const zoneCentroids={};
    for(const f of zonesGJ.features){const z=f.properties.z;if(z){const c=computeCentroid(f.geometry);if(c)zoneCentroids[z]=c;}}
    ...
    map.on('load',async()=>{
      ...
      map.addLayer({id:'zone-fill',type:'fill',source:'zones',filter:['in',['get','ISO_A3'],['literal',countryIsos]],paint:{'fill-color':fillExpr,'fill-opacity':0.38}});
      map.addLayer({id:'zone-hover',type:'fill',source:'zones',filter:['==',['get','z'],''],paint:{'fill-color':fillExpr,'fill-opacity':0.65}});
      ...
      let hovZ=null;
      map.on('mousemove','zone-fill',e=>{map.getCanvas().style.cursor='pointer';const z=e.features[0].properties.z;if(z!==hovZ){hovZ=z;map.setFilter('zone-hover',['==',['get','z'],z]);}popup.setLngLat(e.lngLat).setHTML(`<b>${z}</b><br><span style="opacity:.65;font-size:0.7em">click to explore zone</span>`).addTo(map);});
      map.on('mouseleave','zone-fill',()=>{map.getCanvas().style.cursor='';hovZ=null;map.setFilter('zone-hover',['==',['get','z'],'']);popup.remove();});
      map.on('click','zone-fill',e=>{const z=e.features[0].properties.z;navigate(`/region/${regionId}/results/zone/${encodeURIComponent(z)}`);});
    });
  ...
  const buildMix=()=>{
    const sd=resultsData[scenario];if(!sd||!refYear)return null;
    const zones=countryZoneIds.filter(z=>sd.techFuel[z]?.CapacityTechFuel?.[refYear]);
    if(!zones.length)return null;
    const tfs=[...new Set(zones.flatMap(z=>Object.keys(sd.techFuel[z]?.CapacityTechFuel?.[refYear]||{})))].sort();
    return{labels:zones,datasets:tfs.map(tf=>({label:tf,data:zones.map(z=>Math.round(sd.techFuel[z]?.CapacityTechFuel?.[refYear]?.[tf]||0)),backgroundColor:techColor(tf),borderWidth:0,barThickness:14,stack:'a'}))};
  };

  const buildEvolution=()=>{
    const sd=resultsData[scenario];if(!sd||!allYears.length)return null;
    const zones=countryZoneIds;
    const ind=INDICATORS_C.find(i=>i.key===evIndicator)||INDICATORS_C[0];
    if(ind.source==='yearlyZone'){
      return{labels:allYears,datasets:[{label:scenario,data:allYears.map(y=>+zones.reduce((s,z)=>s+(sd.yearlyZone[z]?.[ind.key]?.[y]||0),0).toFixed(2)),backgroundColor:hexA('#3B82F6',0.75),borderColor:'#3B82F6',borderWidth:2,fill:false,tension:0.3,type:'line'}]};
    }
    const tfs=[...new Set(zones.flatMap(z=>allYears.flatMap(y=>Object.keys(sd.techFuel[z]?.[ind.key]?.[y]||{}))))].sort();
    return{labels:allYears,datasets:tfs.map(tf=>({label:tf,data:allYears.map(y=>Math.round(zones.reduce((s,z)=>s+(sd.techFuel[z]?.[ind.key]?.[y]?.[tf]||0),0))),backgroundColor:hexA(techColor(tf),0.85),borderWidth:0,stack:'a'}))};
  };

  const buildDispatch=()=>{
    const sd=resultsData[scenario];if(!sd||!activeDispZone)return{chartData:{labels:[],datasets:[]},plugin:null};
    ...
    if(dispMode==='full'&&seasons.length&&days.length){
      ...
      const priceData=seasons.flatMap(s=>days.flatMap(d=>Array.from({length:24},(_,h)=>zPrice[s]?.[d]?.[`t${h+1}`]||null)));
      if(priceData.some(v=>v!=null))datasets.push({label:'Marginal cost',type:'line',data:priceData,yAxisID:'yR',borderColor:hexA('#E8C547',0.9),borderWidth:1.5,pointRadius:0,tension:0,fill:false,spanGaps:true});
      const sepPlugin={id:'cDispSep',afterDraw:(chart)=>{
        ...
        for(let si=0;si<nS;si++){const ss=si*nDT*24;ctx.save();ctx.font='700 9px system-ui,sans-serif';ctx.fillStyle=seasC;ctx.textAlign='center';ctx.textBaseline='bottom';ctx.fillText(seasons[si],xS.getPixelForValue(ss+nDT*12),top-2);ctx.restore();
          for(let di=0;di<nDT;di++){const dts=ss+di*24;if(dts>0){const lx=xS.getPixelForValue(dts);const isS=di===0;ctx.save();ctx.strokeStyle=isS?solidC:dashC;ctx.lineWidth=isS?1.2:0.7;if(!isS)ctx.setLineDash([3,3]);ctx.beginPath();ctx.moveTo(lx,top);ctx.lineTo(lx,bottom);ctx.stroke();ctx.restore();}
            const midX=xS.getPixelForValue(dts+12);const w=hoursData?.[seasons[si]]?.[days[di]]||0;const pct=w>0?` (${((w/totalD)*100).toFixed(0)}%)`:'';
  ...
  const mixData=buildMix();const evData=buildEvolution();const dispResult=buildDispatch();
    ...
    <div style={{display:'flex',height:'calc(100vh - 46px)'}}>
      <div style={{position:'relative',flex:1}}>
        ...
        <div style={{position:'absolute',top:10,left:10,zIndex:10,display:'flex',gap:4,alignItems:'center',fontSize:'0.52rem',color:t.text,backgroundColor:t.panel,border:`1px solid ${t.panelBorder}`,borderRadius:5,padding:'4px 10px',boxShadow:'0 1px 4px rgba(0,0,0,.18)'}}>
          ...
          <span style={{color:t.lblMuted}}>›</span><span style={{color:t.lbl,fontWeight:600}}>{countryNameDecoded}</span>
        </div>
      </div>
      <div style={{width:530,flexShrink:0,height:'100%',overflowY:'auto',padding:'18px 16px',backgroundColor:t.panel,borderLeft:`1px solid ${t.panelBorder}`}}>
        ...
        {/* Run selector */}
        <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:12,padding:'7px 10px',border:`1px solid ${t.panelBorder}`,borderRadius:6}}>
          ...
          <select value={scenario||''} onChange={e=>setScenario(e.target.value)} style={{...selectStyle,flex:1}}>
        ...
        {!loading&&hasData&&activeTab==='overview'&&(
          <div style={{display:'flex',flexDirection:'column',gap:14}}>
            {(() => {
              const sd=resultsData[scenario];if(!sd||!refYear)return null;
              const totGW=countryZoneIds.reduce((s,z)=>s+Object.values(sd.techFuel[z]?.CapacityTechFuel?.[refYear]||{}).reduce((a,b)=>a+b,0),0)/1000;
              const demTWh=countryZoneIds.reduce((s,z)=>s+(sd.yearlyZone[z]?.DemandEnergyZone?.[refYear]||0),0)/1000;
            ...
            {mixData&&<div>
              <SectionTitle t={t}>Capacity by zone (MW)</SectionTitle>
              <CJChart type="bar" height={Math.min(mixData.labels.length*22+24,240)} data={mixData} options={{...cjDefaults(t),indexAxis:'y',scales:{x:{stacked:true,grid:{color:t.panelBorder},ticks:{color:t.muted,font:{size:8},callback:v=>v>=1000?`${(v/1000).toFixed(0)}k`:v}},y:{stacked:true,grid:{display:false},ticks:{color:t.muted,font:{size:8}}}}}}/>
            </div>}
          </div>
        )}
        {/* Evolution */}
        {!loading&&hasData&&activeTab==='evolution'&&(
          <div style={{display:'flex',flexDirection:'column',gap:12}}>
            <div style={{display:'flex',gap:6,flexWrap:'wrap',alignItems:'center'}}>
              <select value={evIndicator} onChange={e=>setEvIndicator(e.target.value)} style={selectStyle}>
                {INDICATORS_C.map(ind=><option key={ind.key} value={ind.key}>{ind.label}</option>)}
              </select>
            </div>
            {evData?<CJChart type={INDICATORS_C.find(i=>i.key===evIndicator)?.source==='yearlyZone'?'line':'bar'} height={220} cacheKey={`cev|${scenario}|${refYear}|${evIndicator}`} data={evData} options={{...cjDefaults(t),scales:{x:{stacked:true,grid:{color:t.panelBorder},ticks:{color:t.muted,font:{size:8}}},y:{stacked:true,grid:{color:t.panelBorder},ticks:{color:t.muted,font:{size:8},callback:v=>v>=1000?`${(v/1000).toFixed(0)}k`:v}}}}}/>:<div style={{color:t.lblMuted,fontSize:'0.58rem'}}>No data.</div>}
          </div>
        )}
        {/* Dispatch */}
        {!loading&&hasData&&activeTab==='dispatch'&&(
          <div style={{display:'flex',flexDirection:'column',gap:10}}>
            ...
            <div style={{display:'flex',flexWrap:'wrap',gap:3,alignItems:'center'}}>
              ...
              {dispAvailSeasons.map(s=><Pill key={s} active={dispMode==='season'&&dispSeason===s} onClick={()=>{setDispMode('season');setDispSeason(s);}}>{s}</Pill>)}
            ...
            {dispResult.chartData.datasets.length>0?<>
              <CJChart type="line" height={dispMode==='full'?210:160} data={dispResult.chartData} plugins={dispResult.plugin?[dispResult.plugin]:[]} cacheKey={`cdisp|${scenario}|${activeDispZone}|${dispMode}|${dispSeason}|${dispDay}`}
        ...
        {/* Trade */}
        {!loading&&hasData&&activeTab==='trade'&&(()=>{
          const tx=resultsData[scenario]?.transmission||{};
          if(!refYear)return<div style={{color:t.lblMuted,fontSize:'0.58rem'}}>Select a year.</div>;
          const seen=new Set();const netImports={};
          for(const z of countryZoneIds){let net=0;for(const[z2,attrs]of Object.entries(tx[z]||{}))net-=(attrs.Interchange?.[refYear]||0);for(const[z2,zm]of Object.entries(tx))if(z2!==z)net+=(zm[z]?.Interchange?.[refYear]||0);netImports[z]=+net.toFixed(1);}
          const zones=countryZoneIds.filter(z=>Math.abs(netImports[z]||0)>0).sort((a,b)=>netImports[b]-netImports[a]);
          if(!zones.length)return<div style={{color:t.lblMuted,fontSize:'0.58rem'}}>No trade data.</div>;
          return<div style={{display:'flex',flexDirection:'column',gap:10}}>
            <div style={{fontSize:'0.47rem',letterSpacing:'2px',fontWeight:700,color:t.lblMuted,textTransform:'uppercase',marginBottom:6}}>Net imports by zone (GWh)</div>
            <CJChart type="bar" height={Math.min(zones.length*22+24,220)} cacheKey={`ctr|${scenario}|${refYear}`}
              data={{labels:zones,datasets:[{data:zones.map(z=>netImports[z]),backgroundColor:zones.map(z=>hexA(netImports[z]>=0?'#2E9EC8':'#E8C547',0.75)),borderWidth:0,barThickness:12}]}}
        ...
        {!loading&&hasData&&activeTab==='plants'&&(()=>{
          const pl=(resultsData[scenario]?.plants||[]).filter(p=>p.attribute===plIndicator&&p.y===refYear&&p.value>0&&countryZoneIds.includes(p.z)).sort((a,b)=>b.value-a.value).slice(0,plTopN);
          return<div style={{display:'flex',flexDirection:'column',gap:10}}>
            <div style={{display:'flex',gap:6,flexWrap:'wrap',alignItems:'center'}}>
            ...
            {pl.length>0?<>
              <CJChart type="bar" height={Math.min(pl.length*18+24,280)} cacheKey={`cpl|${scenario}|${refYear}|${plIndicator}|${plTopN}`}
              ...
              />
            </>:<div style={{color:t.lblMuted,fontSize:'0.58rem'}}>No plant data.</div>}
          </div>;
        })()}
      </div>
    </div>
